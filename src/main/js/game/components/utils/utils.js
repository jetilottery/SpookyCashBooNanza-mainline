...
define(function module(require) {    
    ...
    function bindTextures(textures) {
        textures.forEach(function(tex){
            app.renderer.bindTexture(new PIXI.Texture.fromFrame(tex));
        });
    }

    function bindSpine(name) {
        let spine = resLib.spine[name];
        bindTextures(getSpineTextures(spine));
    }

    function getSpineTextures(spine) {
        return spine.children.filter(child => PIXI.utils.TextureCache[child.name] !== undefined).map(child => child.name);
    }
